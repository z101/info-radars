...
class RequestConfig:
    ...
    list_categories_url: str = "https://hackaday.com/"
...
def fetch_available_categories(config: RequestConfig) -> list[tuple[str, str]]:
    fallback = [
        ("3d-printing-hacks", "3D Printing Hacks"),
        ("arduino-hacks", "Arduino Hacks"),
        ("led-hacks", "LED Hacks"),
    ...
    ignored_slugs = {
        "featured", "hackaday-columns", "hackaday-store",
        "hackaday-prize", "kevin-s-files", "editors-picks",
        "originals", "behind-the-scenes", "hackaday-classics",
    }

    try:
        api_url = "https://hackaday.com/wp-json/wp/v2/categories?per_page=100&hide_empty=true"
        headers = {"User-Agent": config.user_agent}
        ...
        for item in data:
            ...
            if slug and slug not in seen and slug not in ignored_slugs:
                seen.add(slug)
                categories.append((slug, name))
        categories.sort()
        return categories if categories else fallback
    except Exception:
        return fallback
