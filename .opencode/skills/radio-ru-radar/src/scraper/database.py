...
INDEXES_SQL = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_search_query_score ON search_scores(query_hash, score)",
...
class Database:
    ...
    def _migrate(self):
        ...
        # Rename total → score in search_scores (if old schema)
        score_cols = [c["name"] for c in self._fetchall("PRAGMA table_info('search_scores')")]
        if "total" in score_cols and "score" not in score_cols:
            self._execute("ALTER TABLE search_scores RENAME COLUMN total TO score")
        # Add rubric_hash / scores_json if missing (migration from incomplete DDL)
        if "rubric_hash" not in score_cols:
            self._execute("ALTER TABLE search_scores ADD COLUMN rubric_hash TEXT NOT NULL DEFAULT ''")
        if "scores_json" not in score_cols:
            self._execute("ALTER TABLE search_scores ADD COLUMN scores_json TEXT")
    ...
    # ------------------------------------------------------------------
    # Search pipeline (single-stage search)
    # ------------------------------------------------------------------
    def get_search_candidates(
        ...
        scope_since_year: int | None = None,
        scope_since_month: int | None = None,
        scope_until_year: int | None = None,
        scope_until_month: int | None = None,
    ) -> list[dict]:
        clauses = [
        ...
        params: list = [query_hash, max_retries]

        if scope_since_year is not None:
            sm = scope_since_month or 1
            clauses.append("AND (a.year > ? OR (a.year = ? AND a.month >= ?))")
            params.extend([scope_since_year, scope_since_year, sm])

        if scope_until_year is not None:
            um = scope_until_month or 12
            clauses.append("AND (a.year < ? OR (a.year = ? AND a.month <= ?))")
            params.extend([scope_until_year, scope_until_year, um])

        clauses.append("ORDER BY a.year DESC, a.month DESC, a.id DESC")
    ...
    def get_search_candidates_batch(
        ...
        all_candidates = self.get_search_candidates(
            query_hash, max_retries,
            scope_limit=scope_limit,
            scope_since_year=scope_since_year, scope_since_month=scope_since_month,
            scope_until_year=scope_until_year, scope_until_month=scope_until_month,
        )
        ...
        self._execute(
            ...
            INSERT INTO search_scores
                (article_id, query_hash, query_name, query_text,
                 rubric_hash, content_hash, status, score, comment, scored_at, attempts)
            VALUES (?, ?, '', '', '', ?, 'scored', ?, ?, ?, 0)
            ON CONFLICT(article_id, query_hash, rubric_hash, content_hash) DO UPDATE SET
                ...
                attempts    = 0
            """,
            (article_id, query_hash, content_hash, relevance, comment, now),
        )
        return True
    ...
    def mark_search_error(
        ...
        if existing:
            ...
            self._execute(
                ...
                INSERT INTO search_scores
                    (article_id, query_hash, query_name, query_text,
                     rubric_hash, content_hash, status, attempts, last_error)
                VALUES (?, ?, ?, ?, '', ?, 'error', 1, ?)
                """,
                (article_id, query_hash, query_name, query_text,
                 content_hash, error),
    ...
    def get_search_report(
        ...
        query = (
            ...
            "WHERE s.query_hash = ? "
            "  AND s.status = 'scored' AND s.score >= ? "
            "ORDER BY a.year DESC, a.month DESC, a.id DESC"
        )
        params: list = [query_hash, min_score]
        if top:
            query += " LIMIT ?"
            params.append(top)
    ...
    def get_search_status(
        ...
        # Build the scope subquery (the set of articles in scope)
        scope_clauses = ["SELECT a.id FROM articles a WHERE 1=1"]
        scope_params: list = []

        if scope_since_year is not None:
            sm = scope_since_month or 1
            scope_clauses.append("AND (a.year > ? OR (a.year = ? AND a.month >= ?))")
            scope_params.extend([scope_since_year, scope_since_year, sm])

        if scope_until_year is not None:
            um = scope_until_month or 12
            scope_clauses.append("AND (a.year < ? OR (a.year = ? AND a.month <= ?))")
            scope_params.extend([scope_until_year, scope_until_year, um])

        scope_clauses.append("ORDER BY a.year DESC, a.month DESC, a.id DESC")
