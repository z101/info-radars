...
class TestSearchPipeline:
    def test_candidates_all_uncached(self, db_path):
        ...
        cands = db.get_search_candidates(qh)
        assert len(cands) == 2
    ...
    def test_scored_article_excluded_from_candidates(self, db_path):
        ...
        cands = db.get_search_candidates(qh)
        art = cands[0]
        db.save_search_result(art["id"], qh, art["content_hash"], 85, "good")
        remaining = db.get_search_candidates(qh)
    ...
    def test_save_result_and_report(self, db_path):
        ...
        cands = db.get_search_candidates(qh)
        art = cands[0]
        db.save_search_result(art["id"], qh, art["content_hash"], 85, "nice article")
        report = db.get_search_report(qh)
        assert len(report) == 1
    ...
    def test_save_result_with_content_hash_disambiguates(self, db_path):
        ...
        db.save_search_result(aid, qh, compute_content_hash("v1", "A1", "И. Автор"), 80, "first")

        db.update_excerpt("A1", 2026, 4, "v2")
        ch2 = compute_content_hash("v2", "A1", "И. Автор")
        db.save_search_result(aid, qh, ch2, 90, "second")

        rows = db._fetchall(
            "SELECT score, comment FROM search_scores WHERE article_id=? ORDER BY score",
            (aid,),
        )
        assert len(rows) == 2
        totals = sorted(r["score"] for r in rows)
        assert totals == [80, 90]
    ...
    def test_candidates_batch(self, db_path):
        ...
        qh = compute_query_hash("q")
        batch0 = db.get_search_candidates_batch(qh, 0, 2)
        assert len(batch0) == 2
        batch1 = db.get_search_candidates_batch(qh, 1, 2)
        assert len(batch1) == 1
    ...
    def test_mark_search_error(self, db_path):
        ...
        cands = db.get_search_candidates(qh)
        art = cands[0]
        db.mark_search_error(art["id"], qh, art["content_hash"], "timeout")
    ...
    def test_error_article_retried(self, db_path):
        ...
        db.mark_search_error(aid, qh, ch, "timeout")
        cands = db.get_search_candidates(qh, max_retries=3)
        assert len(cands) == 1
        db.mark_search_error(aid, qh, ch, "timeout again")
        cands = db.get_search_candidates(qh, max_retries=3)
        assert len(cands) == 1
        db.mark_search_error(aid, qh, ch, "timeout again3")
        cands = db.get_search_candidates(qh, max_retries=3)
        assert len(cands) == 0
    ...
    def test_get_search_status(self, db_path):
        ...
        _seed_article(db, sid)
        qh = compute_query_hash("q")
        status = db.get_search_status(qh)
...
class TestNormalizer:
    def test_normalize_scores_basic(self):
        ...
        normalize_scores(results, key="score")
        assert results[0]["score_normalized"] == 0.0
        assert results[1]["score_normalized"] == pytest.approx(44.4, rel=0.1)
        assert results[2]["score_normalized"] == 100.0
    ...
    def test_normalize_preserves_ordering(self):
        ...
        normalize_scores(results, key="score")
        assert results[0]["score_normalized"] < results[1]["score_normalized"]
        assert results[2]["score_normalized"] < results[1]["score_normalized"]
    ...
    def test_normalize_all_equal(self):
        ...
        normalize_scores(results, key="score")
        for r in results:
            assert r["score_normalized"] == 100.0

    def test_normalize_single_element(self):
        results = [{"score": 42}]
        normalize_scores(results, key="score")
        assert results[0]["score_normalized"] == 100.0

    def test_normalize_empty_list(self):
        assert normalize_scores([], key="score") == []
